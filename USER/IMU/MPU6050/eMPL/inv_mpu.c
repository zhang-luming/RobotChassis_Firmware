...
int mpu_reg_dump(void) {
    ...
    for (unsigned char ii = 0; ii < st.hw->num_reg; ii++) {
 ...
 *  @param[in] Platform-specific parameters to interrupt API.
 *  @return     0 if successful.
 */
int mpu_init(void) {
...
int mpu_configure_fifo(unsigned char sensors) {
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (!(st.chip_cfg.sensors))
            return -1;
        unsigned char prev = st.chip_cfg.fifo_enable;
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
    unsigned short fifo_count = (data[0] << 8) | data[1];
...
static int get_accel_prod_shift(float *st_shift) {
    unsigned char tmp[4], shift_code[3];
...
static int accel_self_test(const long *bias_regular, const long *bias_st) {
    int jj, result = 0;
...
int mpu_run_self_test(long *gyro, long *accel) {
...
#endif
    int result;
    unsigned char accel_fsr, fifo_sensors;
    ...
    unsigned char sensors_on = st.chip_cfg.sensors;
